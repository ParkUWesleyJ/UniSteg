...
# Wesley Jacobs
#
# Drives thje UniSteg and StegEval algorithms to provide
...
def main():
    ...
    while (algorithm_type != "/evaluate" and algorithm_type != "/conceal_extract" and algorithm_type != "/quit"):
        algorithm_type = input(
            colors["invalid"] + "Invalid command.\n" + colors["normal"] +
            "\n---------------------------------------\n" +
            "Enter one of the following commands:\n" +
            " /evaluate\n /conceal_extract\n /quit\n" +
            "---------------------------------------\n\n" +
    ...
    if (type(algorithm) == UniSteg):
        ...
        while (processing_type != "/conceal" and processing_type != "/extract" and processing_type != "/home" and processing_type != "/quit"):
            processing_type = input(    
                colors["invalid"] + "Invalid command.\n" + colors["normal"] + 
                "\n---------------------------------------\n" +       
                "Enter one of the following commands:\n" +
    ...
    # Handles when user wants to evaluate
        ...
        while (eval_type != "/mse" and eval_type != "/psnr" and eval_type != "/qi" 
               and eval_type != "/hist" and eval_type != "/all_evals" and eval_type != "/home" and eval_type != "/quit"):
            eval_type = input(    
                colors["invalid"] + "Invalid input.\n" + colors["normal"] + 
                "\n--------------------------------------\n" +       
                "Enter one of the following commands:\n" +
                " /mse\n /psnr\n /qi\n /hist\n /all_evals\n /quit\n" +
                "---------------------------------------\n\n" +
...
def input_image(num_images, image_array):
    ...
    for i in range(0,num_images):
        ...
        image_path = input(
            "\n---------------------------------------------------------\n" +
            "Enter path of image or enter one of the following commands (Image " + str(len(image_array)+1) + "):\n" +
            " /home\n /quit\n" +
            "---------------------------------------------------------\n\n" +
            "> "
        )
        ...
        while (not os.path.isfile(image_path) and image_path.lower() != "/quit" and image_path.lower() != "/home"):
            image_path = input(
                colors["invalid"] + "Invalid command/path.\n" + colors["normal"] +
                "\n---------------------------------------------------------\n" +
                "Enter path of image or enter one of the following commands (Image " + str(len(image_array)+1) + "):\n" +
                " /home\n /quit\n" +
                "---------------------------------------------------------\n\n" +
        ...
        try:
            ...
            print(colors["invalid"] + "An error occurred. Was the file a PNG/JPEG image? If inputting more than one image, were they the same size?" + colors["normal"])
